...
// const URL: &str = "wss://ftx.us/ws/";
#[tokio::test]
async fn webscoket_test() -> Result<()> {
    let (mut socket, _) = connect_async(Url::parse(URL).expect("Can't update reports")).await?;
    socket.send(Message).await?;
    let msg = socket.next().await.expect("Can't fetch case count")?;
    println!("11111111{:?}", msg);
    socket.close(None).await?;
    Ok(())
}
